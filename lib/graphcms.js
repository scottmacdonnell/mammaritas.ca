...
export async function getDailySpecialsDate(preview) {
  const data = await fetchAPI(
    `
      query GetDailySpecialsDate {
        dailySpecialsDates(first: 1) {
          date
        }
      }
    `,
  ...
  return data.dailySpecialsDates[0].date
}

export async function getRestaurantClosures(preview) {
  const data = await fetchAPI(
    `
      query GetRestaurantClosures {
        restaurantClosures {
          date
          id
          title
  ...
  return data.restaurantClosures
}